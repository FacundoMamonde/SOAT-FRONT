<template>
  <div class="header container-fluid p-0">
    ...
    <b-input-group  >
      <b-form-input v-model="filtro" @input="filtrarTabla" size="sm"></b-form-input>
        <template #append>
        <b-dropdown text="Buscar por" variant="primary">
          <b-dropdown-item @click="filtroPor='cliente'">Cliente</b-dropdown-item>
          <b-dropdown-item @click="filtroPor='equipo'">Equipo</b-dropdown-item>
        </b-dropdown>
      </template>
       </b-input-group>
  ...
  export default {
    ...
    methods: {
      filtrarTabla() {
        bus.$emit("filtro-cambiado", { filtro: this.filtro, filtroPor: this.filtroPor });
      }
    },
  }
  </script>
